    ...
    public void log(ReceivedSensingData msg) throws ResourceNotFoundException,ErrorDetails {
        ...
        if (msg.getMethod().equals("SENSORDATA")){
            ...
            if ( typeName.equals("Temperature")){
                ...
                "Temperature Alarm","Temperature is higher than 40°C ( " + msg.getValue() + "°C )",
                    msg.getTimestamp(),sensor.getDivision().getHome());

                }else if (msg.getValue()<10){
                    notification = new Notification(0,
                "Temperature Alarm","Temperature is lower than 10°C ( " + msg.getValue() + "°C )",
                    msg.getTimestamp(),sensor.getDivision().getHome());
                }
            }
                ...
                if(msg.getValue() > 80){
                    notification = new Notification(0,
                "Humidity Alarm","Humidity is higher than 80% ( " + msg.getValue() + "º% )",
                    msg.getTimestamp(),sensor.getDivision().getHome());

                }else if (msg.getValue()<10){
                    notification = new Notification(0,
                "Humidity Alarm","Humidity is lower than 10% ( " + msg.getValue() + "%)",
                    msg.getTimestamp(),sensor.getDivision().getHome());
                }
            }
                ...
                if(msg.getValue() > 85){
                    notification = new Notification(0,
                "Luminosity Alarm","Luminosity is lower than 85% ( " + msg.getValue() + "% )",
                    msg.getTimestamp(),sensor.getDivision().getHome());
                }else if (msg.getValue()<15){
                    notification = new Notification(0,
                "Luminosity Alarm","Luminosity is lower than 15% ( " + msg.getValue() + "% )",
                    msg.getTimestamp(),sensor.getDivision().getHome());
                }
            }
